...
# [GPU/MPS] Mixed precision training - conditional import for MPS compatibility
try:
    from torch.cuda.amp import autocast, GradScaler
    AMP_AVAILABLE = True
except ImportError:
    AMP_AVAILABLE = False
    print("Warning: AMP not available, training without mixed precision")
...
def train(data_pairs, model, config_args, train_args):
	...
	# [GPU/MPS] Initialize mixed precision scaler - only for CUDA
	scaler = GradScaler() if (device.type == 'cuda' and AMP_AVAILABLE) else None
	...
	for epoch in epoch_pbar:
		...
		for it, batch in enumerate(batch_pbar):
			...
			# [GPU/MPS] Mixed precision forward pass - only use autocast with CUDA
			...
			# MPS synchronization for stability
			if device.type == 'mps':
...
# Apply MPS fixes before using the device
try:
    from mps_fix import apply_mps_fixes
    apply_mps_fixes()
except ImportError:
    pass
