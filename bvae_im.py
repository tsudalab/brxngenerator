    ...
    def decode_many_times(self, latent):
        ...
        for i in range(5000):
            if len(product_list) > 5:
                break
            latent_new = torch.cat([latent, torch.randint(0, 2, (latent.shape[0], latent.shape[1] * 2 - latent.shape[1]))])
    ...
    def _build_qubo(self, configs):
        ...
        for epoch in range(configs['opt']['maxepoch']):
            ...
            with torch.no_grad():
                ...
                epoch_error = criterion(self.y_test, y_hat_test)
                r2_test = r2_score(self.y_test.numpy(), y_hat_test.numpy())
...
if __name__ == "__main__":
    ...
    if metric == "qed":
        for i, data_pair in enumerate(data_pairs):
            latent = model.fragment_encode([data_pair])
