...
class GurobiQuboSolver(BaseQuboSolver):
    ...
    def solve(self, model: TorchFM):
        ...
        # 获取模型训练好的参数
        V = model.factor_matrix.detach().numpy()  # shape (n, k)
        W = model.lin.weight.detach().numpy().flatten()  # shape (n,)

        # 1. 计算包含所有交互项的矩阵 (V * V^T)
        # 这一步计算了所有 i, j 对（包括 i=j）的点积
        interaction_matrix = V @ V.T

        # 2. 创建 Q 矩阵
        # 先用交互矩阵填充
        Q = interaction_matrix
        ...
        print("\nLinear Weights W:\n", W)
        print("\nResulting QUBO Matrix Q:\n", Q)
    
    
class FactorizationMachineSurrogate:
    def __init__(self, n_features, k_factors, ):
        self.device = DEVICE
        self.model = TorchFM(n_features, k_factors).to(self.device)
        self.n_binary = n_features
        self.k_factors = k_factors
        # ... 其他初始化 ...

    def train(self, X_train, y_train, X_test, y_test):
            model = TorchFM(self.n_binary, FACTOR_NUM).to(DEVICE)
            ...
            for p in model.parameters():
                if tuple(p.shape) == (self.n_binary, FACTOR_NUM):
    ...
    def to_qubo(self):
        # 把 _build_qubo() 中构建 QUBO 的代码移到这里
...
class MoleculeOptimizer:
    def __init__(self, bvae_model, initial_data, surrogate_model, qubo_solver, configs):
        """
        通过依赖注入的方式，接收各个组件
        """
        ...
        # 数据和状态
        self.X_train, self.y_train, self.X_test, self.y_test = initial_data
    ...
    def optimize(self):
        self.iteration = 0
        n_opt = self.configs['opt']['num_end']
        ...
        while self.iteration < n_opt:
            ...
            # 2. 从代理模型构建 QUBO 问题
            qubo_problem = self.surrogate.to_qubo()
            
            # 3. 使用注入的求解器求解
            solutions, energies = self.solver.solve(qubo_problem)
    ...
    def _update(self, solutions, energies):
        # _update 的逻辑基本不变，但它现在只负责更新数据集
        # ...
        pass
    
    def _decode_and_score(self, binary_vector):
        # 可以把 decode_many_times 和后续的评分逻辑也抽成一个独立的函数或类
        pass
...
#     '''This class is used to optimize the binary vector using the bVAE model. It uses the FixstarsClient to solve the QUBO problem.(just now, maybe able to use other api in the future)'''
...
#             # F.one_hot encodes the latent vector to binary,no problems
...
def prepare_dataset(metric="qed",molecules=None):
